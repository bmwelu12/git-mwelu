...
const TerminalLayout = () => {
 ...
 const handleInputSubmit = (e: { preventDefault: () => void }) => {
  ...
  const inputString = (
   <p className='text-white font-mono' key={uniqueKey + '-input'}>
    ...
    <span className='text-blue-500'>bmwelu.sh</span>
    <span className='text-yellow-500'> ~ $ </span>
    {sanitizedInput}
   </p>
  );
 ...
 const formatUserCount = () => {
  ...
  return `bmwelu${paddedCount}`;
 };
 const openCV = () => {
  // Open the CV in a new tab
  window.open('/BREDA_MWELU_RESUME.pdf', '_blank');
 };
 return (
  <div className='w-full md:w-3/4 bg-black text-green-500 p-6 rounded-lg shadow-lg mx-auto'>
   {booting ? (
    ...
    <>
     {/* bnira.sh and say hi badge at the far right */}
     <div className='flex justify-between items-center mb-4'>
      <p className='text-green-500 text-2xl font-mono'>bmwelu.sh</p>
      <button className='bg-blue-400 text-white px-4 py-2 rounded' onClick={openCV}>My Resume</button>
     </div>
     ...
     <p className='text-white font-mono mb-4'>
                        I am Breda Mwelu, I am currently a senior Computer Science and French student at Drew University New Jersey
     </p>
